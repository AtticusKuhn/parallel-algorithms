...
open import Felix.Raw using (Category ; Cartesian ; Cocartesian ; id ; _∘_ ; _▵_ ; twice ; _⊗_ ; inl ; inr ; ! ; _▿_ ; exl ; exr ; first ; second ; assocʳ ; assocˡ ;inAssocʳ′ ; transpose ;  dup ; swap ;   unitorⁱʳ ; constʳ)
...
-- LittleEndianToNat 1 = if ∘ constʳ (p1 ▵ p0) ∘ exl
LittleEndianToNat (suc b) =   add ∘ ((if ∘ constʳ (p1 ▵ p0))  ⊗ (*2 ∘ LittleEndianToNat b ))   --  ∘  second (*2 ∘ LittleEndianToNat b)     -- if ∘ second ( first *2 ∘ twice (LittleEndianToNat b) ∘ dup) --- ((id) ▿ (*2 hpa)) ⊗ (LittleEndianToNat b)
```
...
fullAdder = swap ∘   first or ∘ inAssocʳ′ (second (swap ∘  halfAdder))  ∘ first (swap ∘ halfAdder) -- (or bitops ∘  ( exl ∘ halfAdder ∘ exl ▵  exl ∘ halfAdder ∘ first (exr ∘ halfAdder))) ▵  exr ∘ halfAdder ∘ first ( exr ∘ halfAdder)
```
